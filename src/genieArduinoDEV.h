...
class Genie {
  public:
    ...
    void        debug                (Stream &serial) { debugSerial = &serial; }
    bool        online               () { return displayDetected; }
    int8_t      form                 () { return currentForm; }
    void        form                 (uint8_t newForm) { WriteObject(GENIE_OBJ_FORM, newForm, (uint16_t)0x0000); }
    void        recover              (uint8_t pulses) { recover_pulse = pulses; }
    ...
    bool        contrast             (uint8_t value) { return WriteContrast(value); }
    bool        WriteContrast        (uint8_t value);
    bool        WriteStr             (uint8_t index, const char *string);
    bool        WriteStr             (uint8_t index, String string) { return WriteStr(index,string.c_str()); }
    ...
    bool        WriteInhLabel        (uint8_t index, String string) { return WriteInhLabel(index,string.c_str()); }
    ...
    uint16_t    WriteInhLabel        (uint16_t index, double n) { return WriteInhLabel(index, n, 2); }
#ifdef AVR
    uint16_t	WriteInhLabel		(uint16_t index, const __FlashStringHelper *ifsh);
#endif
    uint8_t     EventIs              (genieFrame * e, uint8_t cmd, uint8_t object, uint8_t index) { return (e->reportObject.cmd == cmd && e->reportObject.object == object && e->reportObject.index == index); }
    uint16_t    GetEventData         (genieFrame * e) { return (e->reportObject.data_msb << 8) + e->reportObject.data_lsb; }
    void        DequeueEvent         (genieFrame * buff);
    int16_t     DoEvents             ();
    void        Ping                 (uint16_t interval);
    void        AttachEventHandler             (UserEventHandlerPtr userHandler);
    void        AttachMagicByteReader          (UserBytePtr userHandler) { UserByteReader = userHandler; }
    void        AttachMagicDoubleByteReader    (UserDoubleBytePtr userHandler) { UserDoubleByteReader = userHandler; }
    uint32_t    uptime                         () { if ( displayDetected ) return millis() - display_uptime; else return 0; }
    ...
    int16_t   GetNextByte         (void);
    int32_t   GetNextDoubleByte   (void);
